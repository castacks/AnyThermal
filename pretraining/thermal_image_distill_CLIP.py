...
parser.add_argument('--save_path', default='./checkpoints_ce/clip_ms2_checkpoints_thermal_global_bigger_denser_no_night', type=str, help='Path to save the checkpoints')
...
parser.add_argument('--train_both', default=False, type=bool, help='Train both RGB and thermal models')
args = parser.parse_args()
print(args)

#Initialize wandb
if args.wandb_use:
    wandb.init(project="multiloc", entity="jkarhade", name="clip_thermal_image_distill")

# Load models
rgb_model = timm.create_model("vit_small_patch16_224",pretrained=True, num_classes=0).cuda()
rgb_model.eval()
thermal_model = timm.create_model("vit_small_patch16_224",pretrained=True, num_classes=0).cuda()
thermal_model.eval()
...
def train():
    ...
    if args.dataset == "ms2":
        ...
        dataset = CART_dataset(args.dataset_path, model_type="clip")
        # x = dataset[5732]
