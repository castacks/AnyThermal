...
from ms2_utils import load_as_float_img, load_as_float_depth, get_transform, process_image
...
class DataLoader_MS2(data.Dataset):
    ...
    * Structure of "KAIST MS2 dataset"
    ...
    def __init__(
        ...
        self.blur_random_rate = None
        # blur = ksize:rate
        if blur is not None and len(blur.split(":")) == 2:
            self.blur_ksize = int(blur.split(":")[0])
            self.blur_random_rate = float(blur.split(":")[1])
            print(f"blur with kernal {self.blur_ksize}x{self.blur_ksize} at {self.blur_random_rate*100}% probability")
    ...
    def __getitem__(self, index):
        if isinstance(self.modality, list):
            ...
            if "nir" in self.modality:
                results["nir"] = self.data_getter(index, "nir")
    ...
    def set_extrinsics(self):
        ...
        ext_NIR2THR = np.concatenate(
            [calib["R_nir2thr"], calib["T_nir2thr"] * 0.001], axis=1
        )  # mm -> m scale conversion.
        ...
        for sensor in sensor_list:
            ...
            for seq in self.seq_list:  # iterate over each sequence
                ...
                if sensor == "rgb":
                    ...
                    extrinsics_L2R = np.concatenate([calib["R_rgbR"], calib["T_rgbR"] * 0.001], axis=1).astype(
                        np.float32
                    )
                    baseline = abs(calib["T_rgbR"][0].astype(np.float32)) * 0.001  # convert to the meter scale
                elif sensor == "nir":
                    intrinsics = calib["K_nirL"].astype(np.float32)
                    extrinsics_L2R = np.concatenate([calib["R_nirR"], calib["T_nirR"] * 0.001], axis=1).astype(
                        np.float32
                    )
                    baseline = abs(calib["T_nirR"][0].astype(np.float32)) * 0.001
                elif sensor == "thr":
                    intrinsics = calib["K_thrL"].astype(np.float32)
                    extrinsics_L2R = np.concatenate([calib["R_thrR"], calib["T_thrR"] * 0.001], axis=1).astype(
                        np.float32
                    )
                ...
                for i in range(init_offset, len(img_list_left) - init_offset):
                    ...
                    sample = {
                        ...
                        "tgt_img_left": img_list_left[i],
                        "tgt_img_right": img_list_right[i],
                        "ref_imgs_left": [],
                        "ref_imgs_right": [],
                        "tgt_depth_in": depth_in,
                        "tgt_depth_gt": depth_gt,
                        "tgt_inten_in": inten_in,
                        "ref_intens_in": [],
                        "ref_depths_in": [],
                        "ref_depths_gt": [],
                        "baseline": baseline,
                        "extrinsics_L2R": extrinsics_L2R,
                    }
    ...
    # Just to get the thr data
    def get_thr_sample(self, index, modality):
        sample = self.samples["modality"][index]
        tgt_img = load_as_float_img(sample["tgt_img_left"])
    ...
    # For Lidar-Visual-Thermal Distillation

    def get_data_Distill(self, index, modality):
        sample = self.samples[modality][index]
    ...
    def get_data_StereoMatching(self, index, modality):
        ...
        # resize all images and depth maps to wxh. They are numpy arrays.
        tgt_img_left = cv2.resize(tgt_img_left, (w, h))
        tgt_img_left = np.expand_dims(tgt_img_left, axis=2)
        tgt_img_right = cv2.resize(tgt_img_right, (w, h))
        tgt_img_right = np.expand_dims(tgt_img_right, axis=2)
        disparity_gt = cv2.resize(disparity_gt.numpy(), (w, h), interpolation=cv2.INTER_NEAREST)
        disparity_gt /= 640 / w

        tgt_img_left, tgt_img_right = process_image(tgt_img_left, tgt_img_right, self.process)

        # grayscale to RGB
        tgt_img_left = cv2.cvtColor(tgt_img_left, cv2.COLOR_GRAY2RGB)
        tgt_img_right = cv2.cvtColor(tgt_img_right, cv2.COLOR_GRAY2RGB)

        # blur
        if self.blur_random_rate is not None:
            if random.random() < self.blur_random_rate:
                # left or right 50% probability
                if random.random() < 0.5:
                    tgt_img_left = cv2.GaussianBlur(tgt_img_left, (self.blur_ksize, self.blur_ksize), 0)
                else:
                    tgt_img_right = cv2.GaussianBlur(tgt_img_right, (self.blur_ksize, self.blur_ksize), 0)

        transforms = get_transform()
        tgt_img_left = transforms(tgt_img_left)
        tgt_img_right = transforms(tgt_img_right)

        disparity_gt_low = cv2.resize(disparity_gt, (w // 4, h // 4), interpolation=cv2.INTER_NEAREST)

        result = {}
        result["left"] = tgt_img_left
        result["right"] = tgt_img_right
        result["disparity"] = torch.as_tensor(disparity_gt)
        result["disparity_low"] = torch.as_tensor(disparity_gt_low)
        return result
    ...
    def get_data_Odometry(self, index, modality):
        ...
        pose = np.stack(
            [np.genfromtxt(pose).astype(np.float64).reshape(4, 4)[:3, :] for pose in sample["poses"]]
        )  # 3x4
