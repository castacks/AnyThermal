...
def evaluate_segmentation(model, dataloader, num_classes):
    ...
    with torch.no_grad():
        for batch, index in tqdm(dataloader, desc="Evaluating"):
            images, masks = batch["thr_seg"].to(model.device), batch["seg_mask"].to(model.device).long()
...
class BenchmarkSegArgs:
    ...
    dataset_name: Literal['cart'] = 'cart'
    batch_size: int = 4
    use_wandb: bool = True
    ...
    splits: List[Literal['train', 'val', 'test']] = field(default_factory=lambda: ['test'])
...
def main(args: BenchmarkSegArgs):
    ...
    for split in args.splits:
        for area in args.test_areas:
            wandb_name = f"segmentation_benchmark_{args.dataset_name}_{args.model_names}_{split}_{area}"
            wandb.init(project="Segmnetation Benchmark", config=vars(args), name=wandb_name)

            print(f"Evaluating on area: {area}, split: {split}")

            if args.dataset_name == "cart":
                seq_list = return_cart_split_segmentation_geographic(split, area, "thermal")
                dataset = CART(root_frame_dir=None, db_modality="thr_seg", q_modality="seg_mask", datasets_folder=None, seq=seq_list, augment=False, seq_as_txt="thermal",crop_images=False)
            else:
            ...
            root_out_dir = os.path.join(args.output_dir, 'segmentation', args.dataset_name, area, split, timestamp)
            ...
            with open(mean_metrics_csv, 'w', newline='') as mm_file, \
                ...
                for model_name in args.model_names:
                    ...
                    print(f"[Area: {area} | Split: {split} | Model: {model_name}] Pixel Acc: {pixel_acc:.4f}, Mean Acc: {mean_acc:.4f}, mIoU: {miou:.4f}, FWIoU: {fwiou:.4f}")
