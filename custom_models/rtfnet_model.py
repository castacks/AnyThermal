...
class RTFNet(nn.Module):
    ...
    def __init__(self, n_class,num_resnet_layers):
        ...
        if self.num_resnet_layers == 18:
            ...
            raise ValueError("Unsupported number of ResNet layers: {}".format(self.num_resnet_layers))
...
class RTFNetModel(BaseSegmentationModel):
    def __init__(self, num_classes, num_resnet_layers, device='cuda'):
        self.num_resnet_layers = num_resnet_layers
        super(RTFNetModel, self).__init__(num_classes =num_classes, device =device)
    def build_model(self):
        model = RTFNet(self.num_classes, self.num_resnet_layers)
