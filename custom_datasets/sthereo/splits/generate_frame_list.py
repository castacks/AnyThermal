...
# === CLI Arguments ===
parser = argparse.ArgumentParser(description="Generate RGB–Thermal frame pairs with GPS")
parser.add_argument(
    "--root_dir",
    type=Path,
    required=True,
    help="Root directory containing STHEREO sequences"
)
...
sequence_dirs = [
    p.parent.parent
    for p in root_dir.glob("*/image/stereo_left")
    if (p.parent / modal_thm).exists()
]
